"""
Stage 4.5 + Stage 5 + Stage 5.5 + Stage 5.6 -- Delayed init near the
...
for disambiguating close, similar-looking objects, and a per-signal
RELIABILITY layer laying the groundwork for position fusion.

This file is a full copy of stage5_5_context.py (kept as a separate,
independently runnable file, matching how that file was itself kept
separate from stage5_redetection.py) with Stage 5.6 built on top. See the
"Position-fusion reliability" section further down for what Stage 5.6
Stage 1 adds and why; everything above it is unchanged from
stage5_5_context.py.

Stage 5.6's own plan has four stages:
  1. Reliability score per signal (THIS file, so far) -- measure only, no
     fusion yet. Compute a normalized [0-1] weight per frame for each
     independent position signal (VitTrack, ORB re-detection, supporter
     consensus, GMC motion), log them all alongside their positions, and
     see how they actually behave on real footage before trusting any of
     them to correct anything.
  2. Weighted fusion during confirmed TRACKING, guarded: VitTrack stays the
     anchor; a correction only applies when a secondary signal disagrees
     strongly AND persistently AND is clearly more reliable.
  3. One unified output position + quality value, continuous across
     TRACKING and LOST/re-detection states.
  4. Tuning + confirmed graceful degradation + real-time budget check.
Only stage 1 is implemented here so far.
...
Usage:
    python stage5_6.py --video "../ex/track-train.mp4"
    python stage5_6.py --video "../ex/track-train.mp4" --x 960 --y 540
"""
...
BOX_SIZE = 40
WINDOW_NAME = "Stage 5.6 - Re-detection + Context-Aware Tracking + Reliability"
...
# -- Stage 5.6, Stage 1: per-signal reliability (measure only, no fusion) --
# Each independent position signal gets a normalized [0-1] reliability
# weight computed fresh every frame it's available: VitTrack (its own
# score), ORB re-detection (match count + cluster tightness), supporter
# consensus (active-vote count + vote agreement), and GMC motion (RANSAC
# inlier ratio + decay for how long a position estimate has relied on pure,
# unanchored motion propagation). Nothing is fused or corrected on any of
# this yet -- purely a measurement layer (see vit/orb/consensus/gmc
# _reliability() functions and their call sites), logged so real behavior
# on this footage can be inspected before trusting any of it to correct
# anything (that's Stage 5.6's Stage 2+).
ORB_RELIABILITY_MATCH_SATURATION = 50   # ORB matches at/above this count toward "fully reliable" on
                                    # the match-count axis (diminishing returns beyond it)
ORB_RELIABILITY_SPREAD_SCALE = 100  # px; matched-keypoint spread (tightness) at/above this pushes the
                                    # tightness factor toward 0. Calibrated from measured data, not
                                    # guessed: an initial value of 15px made this reliability near-0
                                    # almost everywhere (150-sample check across 5 points: min=7.9,
                                    # p10=22.9, p25=31.4, median=48.5, p75=79.3, p90=102.2, max=150.1) --
                                    # real match clusters routinely spread 20-130px here, unsurprising
                                    # since CANDIDATE_CLUSTER_DISTANCE=60px is already how far apart
                                    # points can be and still count as "one" candidate. 100 puts the
                                    # measured median (~48px) at a genuinely moderate ~0.5 instead of
                                    # bottoming out, while the worst quartile (>=100px) still reads near 0.
CONSENSUS_RELIABILITY_VOTE_SATURATION = SUPPORTER_COUNT  # active votes at/above this count toward
                                    # "fully reliable" on the vote-count axis
CONSENSUS_RELIABILITY_SPREAD_SCALE = SUPPORTER_VOTE_TOLERANCE  # px; vote spread at/above this pushes
                                    # the agreement factor toward 0 (reuses the same scale already
                                    # used to call a vote an outlier)
GMC_RELIABILITY_DECAY_HALF_LIFE = 60  # frames of continuous pure-motion reliance (no fresh confirmed
                                    # anchor) after which GMC's own reliability has halved -- drift
                                    # compounds the longer nothing re-anchors the estimate

# -- Stage 5.6, Stage 2: guarded weighted fusion during confirmed TRACKING --
# VitTrack remains the anchor. A correction is only ever applied when ALL
# three guards pass simultaneously: strong disagreement, a clearly higher-
# reliability secondary signal, and that combination persisting for several
# consecutive frames (not a single noisy frame). This is deliberately
# conservative -- a bad signal must never drag down a good one, and the
# whole point of "guarded" is that doing nothing is the default, correction
# is the rare exception that requires real, sustained evidence.
FUSION_MIN_DISAGREEMENT_PX = 60      # px; matches SUSPICIOUS_CONSENSUS_DIST's sense of "a different
                                    # location", not ordinary jitter -- kept as its own constant so the
                                    # monitoring-only flag and the actual correction guard can be tuned
                                    # independently later
FUSION_MIN_RELIABILITY_MARGIN = 0.15  # consensus reliability must exceed VitTrack's own by at least
                                    # this much -- "clearly" more reliable, not just marginally ahead
FUSION_PERSISTENCE_FRAMES = 5        # the combined guard condition must hold this many CONSECUTIVE
                                    # frames before a correction is applied -- the guard against a
                                    # single-frame nudge/feedback loop

# Same guarded fusion, extended to TENTATIVE (probation) -- user observation:
# during probation, VitTrack's own regression on a just re-initialized
# candidate can be noisier than the supporter consensus, which has been
# tracked continuously and independently of whether the candidate itself
# is even the right object. Uses the SAME disagreement/margin guards as
# confirmed tracking (still "clearly better, not just ahead"), but its own
# SEPARATE streak/history so it doesn't interact with the confirmed-
# tracking guard's timing, and a SHORTER persistence requirement --
# probation episodes are often brief (many fail within 3-10 frames, well
# before FUSION_PERSISTENCE_FRAMES=5 could ever accumulate), so a 5-frame
# bar would rarely get a chance to fire at all in this window.
PROBATION_FUSION_PERSISTENCE_FRAMES = 2

# TENTATIVE candidates are, by definition, not yet confirmed -- VitTrack's
# own score on them is inherently lower than on a settled, confirmed lock
# (measured: vit_rel 0.21-0.32 on real corrections here, vs. 0.3-0.9+ typical
# once confirmed). Demanding the SAME strict disagreement/margin bar as
# confirmed TRACKING means the guard almost never has room to trip during
# probation at all. Loosened (broadened) specifically for probation: once
# the object is confirmed, the strict FUSION_MIN_* bars above take back over.
PROBATION_FUSION_MIN_DISAGREEMENT_PX = 40      # was 60 (shared bar) -- still well above ordinary jitter
PROBATION_FUSION_MIN_RELIABILITY_MARGIN = 0.10  # was 0.15 (shared bar) -- still requires consensus to be
                                    # clearly ahead, not just marginally, given the smaller gap available
                                    # when both signals sit in a lower range during probation

CONSENSUS_RELIABILITY_SMOOTHING_WINDOW = 5  # frames averaged to get the SMOOTHED consensus
                                    # reliability the fusion guard actually uses (raw, instantaneous
                                    # consensus_reliability() is still logged separately for
                                    # comparison). Why: measured directly -- across 7 test points, the
                                    # guard's disagreement streak NEVER exceeded 2 (of the 5 required),
                                    # even during a real, sustained drift episode, because raw
                                    # consensus reliability swung wildly frame to frame (observed
                                    # 0.79->0.59->0.39->0.28->0.24->0.00 in under 10 frames) -- a real,
                                    # persistent problem was being masked by short-term reliability
                                    # noise before the persistence counter could ever accumulate.
...
def estimate_motion_with_inliers(prev_gray, curr_gray, feature_mask_small, scale=GMC_DOWNSCALE):
    """Same estimation as stage4_gmc.estimate_motion, but ALSO returns the
    RANSAC inlier ratio (inliers / matches fed into the affine fit) --
    estimate_motion itself discards this (its own second return value is
    the PRE-RANSAC match count, not a post-fit inlier count), and changing
    its return signature would break the OTHER stage files that already
    unpack it as a 2-tuple. This is a self-contained local duplicate of the
    same steps instead of a shared-file change, kept in this file only.
    Returns (M, n_matches, inlier_ratio); M is None (inlier_ratio 0.0) if
    too few matches survive to trust the estimate at all."""
...
def bbox_has_valid_shape(bbox):
    """True if `bbox` has positive width/height -- excludes VitTrack's
    known degenerate-failure box (0,0,0,0), whose "center" would otherwise
    compute to (0,0) -- the frame's top-left corner, not a real reported
    position. Guards the fusion logic (Stage 5.6) from treating that
    meaningless point as a genuine competing signal: measured this exact
    bug directly -- 7 of 11 "corrections" in one real test run were the
    guard reacting to a degenerate (0,0) box being enormously "far" from
    consensus, not a real disagreement between two valid positions."""
    return bbox is not None and bbox[2] > 0 and bbox[3] > 0


def vit_reliability(score):
    """VitTrack's own confidence score, clipped to [0,1] defensively (it
    already lives there in practice)."""
    return clip01(score)


def orb_reliability(n_matches, spread):
    """Combines match COUNT (more matches = more confidence there's a real
    match at all) with match TIGHTNESS (a spread-out cluster of "matches"
    localizes the candidate poorly even if there are many of them) --
    MULTIPLIED, not averaged, so a candidate needs to be reasonably strong
    on BOTH axes to score as reliable; a high count with a wide spread, or
    a tight cluster with almost no matches, should each independently be
    able to drag this down."""
    count_factor = clip01(n_matches / ORB_RELIABILITY_MATCH_SATURATION)
    tightness_factor = clip01(1.0 - spread / ORB_RELIABILITY_SPREAD_SCALE)
    return count_factor * tightness_factor


def consensus_reliability(n_votes, spread):
    """Combines active-supporter COUNT with vote AGREEMENT (tight
    agreement = high confidence in the consensus position) -- multiplied
    for the same reason as orb_reliability: wide disagreement OR few
    supporters should each independently be able to drag this down, not
    just be averaged away by the other looking fine."""
    count_factor = clip01(n_votes / CONSENSUS_RELIABILITY_VOTE_SATURATION)
    agreement_factor = clip01(1.0 - spread / CONSENSUS_RELIABILITY_SPREAD_SCALE)
    return count_factor * agreement_factor


def gmc_reliability(inlier_ratio, frames_since_anchor):
    """Combines this frame's own RANSAC inlier ratio (how well the affine
    model actually fits the matched points) with an exponential DECAY the
    longer the current position estimate has relied on pure, unanchored
    motion propagation -- each additional frame of chained estimation
    compounds drift (see the Stage 4.5 held-point drift history earlier in
    this project), so trust in a long-unanchored motion estimate should
    fall over time even if any single frame's own inlier ratio looks fine."""
    fit_factor = clip01(inlier_ratio)
    decay_factor = 0.5 ** (frames_since_anchor / GMC_RELIABILITY_DECAY_HALF_LIFE)
    return fit_factor * decay_factor


def compute_fused_position(vit_pos, vit_rel, consensus_pos, consensus_rel, disagreement_streak,
                            min_disagreement=FUSION_MIN_DISAGREEMENT_PX,
                            min_reliability_margin=FUSION_MIN_RELIABILITY_MARGIN,
                            persistence_frames=FUSION_PERSISTENCE_FRAMES):
    """Stage 5.6, Stage 2: guarded weighted fusion during confirmed
    TRACKING. VitTrack remains the anchor -- the output only moves away
    from it when ALL of:
      1. VitTrack and consensus disagree by at least `min_disagreement` px
         (a small gap does nothing -- VitTrack wins outright).
      2. Consensus reliability exceeds VitTrack's own by at least
         `min_reliability_margin` -- "clearly" more reliable, not merely
         ahead by noise.
      3. That combined condition (1 AND 2) has held for
         `persistence_frames` CONSECUTIVE frames already -- guards against
         a single noisy frame triggering a correction/feedback loop.
    Only once all three hold does the output become the reliability-
    weighted average of the two positions (NOT a hard snap to consensus --
    the correction magnitude still reflects relative confidence).

    Returns (fused_pos, corrected, new_streak). If consensus is
    unavailable, returns (vit_pos, False, 0) -- VitTrack untouched, streak
    reset (see caller: the streak should only accumulate across
    consecutive frames within the SAME confirmed-tracking episode)."""
    if consensus_pos is None:
        return vit_pos, False, 0

    dist = math.hypot(vit_pos[0] - consensus_pos[0], vit_pos[1] - consensus_pos[1])
    guard_met = dist >= min_disagreement and consensus_rel >= vit_rel + min_reliability_margin
    new_streak = disagreement_streak + 1 if guard_met else 0

    if new_streak < persistence_frames:
        return vit_pos, False, new_streak

    total_rel = vit_rel + consensus_rel
    if total_rel <= 0:
        return vit_pos, False, new_streak
    fx = (vit_rel * vit_pos[0] + consensus_rel * consensus_pos[0]) / total_rel
    fy = (vit_rel * vit_pos[1] + consensus_rel * consensus_pos[1]) / total_rel
    return (fx, fy), True, new_streak
...
def orb_locate_candidates(orb, bf, search_gray, template_entries, box_size, offset=(0, 0),
    ...
    in the full frame), sorted by n_matches descending. `box_size` = (w, h)
    of the tracking box to place at each matched location -- NOT derived
    from the spread of matched keypoints for BOX SIZING (tried and found to
    blow up unboundedly: weak/generic matches scatter across a wide area,
    and feeding that inflated size back into the next template crop
    compounds into a runaway growing box, the same failure shape as the
    Stage 2 degenerate full-frame lock). `spread` (mean distance of the
    cluster's own matched points from their median) IS reported, though --
    not used for box sizing, but as a RELIABILITY signal (Stage 5.6, Stage
    1): a tight cluster of matches localizes the candidate confidently, a
    loosely scattered one is a weaker appearance match even with the same
    raw count.
...
def compute_supporter_consensus(supporters, overlay_mask=None, line_margin=SUPPORTER_LINE_EXCLUSION_MARGIN,
    ...
    their own median) is a RELIABILITY signal (Stage 5.6, Stage 1): tight
    agreement among supporters means high confidence in the consensus,
    wide disagreement means low confidence even with the same vote count.
...
def draw_final_output(frame, final_pos, final_quality, dominant_signal, color=(255, 0, 255)):
    """Stage 5.6, Stage 3: the ONE unified output position -- a tilted-cross
    marker distinct from every other overlay (VitTrack's own box, the
    consensus diamond, the GMC prediction cross), since this is the
    system's actual final decision each frame. May equal VitTrack's own
    position (the common case) or differ from it (only after a guarded
    fusion correction, or during LOST/HOLDING where VitTrack has no
    opinion at all) -- `dominant_signal` says which."""
    x, y = int(final_pos[0]), int(final_pos[1])
    cv2.drawMarker(frame, (x, y), color, markerType=cv2.MARKER_TILTED_CROSS, markerSize=24, thickness=3)
    q_str = f"{final_quality:.2f}" if final_quality is not None else "N/A"
    cv2.putText(frame, f"FINAL q={q_str} [{dominant_signal}]", (x + 14, y + 22),
...
def main():
    ...
    n_fusion_corrections = 0
    consensus_reliability_history = []
    ...
    while True:
        ...
        if final_pos is not None:
            draw_final_output(display, final_pos, final_quality, dominant_signal)
        ...
        if supporters:
            ...
            supporter_motion_M, _, _ = estimate_motion_with_inliers(prev_frame_gray, curr_frame_gray, flow_mask_small)
        ...
        gmc_rel = None  # reset every frame -- stays None unless freshly computed just below (Stage
                        # 5.6, Stage 3 reads this for the LOST-state final-output quality value, and
                        # a stale value from a previous frame would misreport this frame's confidence)
        ...
        if uncertain and lost_prev_gray is not None and predicted_pos is not None:
            ...
            # Stage 5.6, Stage 1: GMC reliability -- combines this frame's own
        ...
        # Stage 5.6, Stage 3: ONE unified final output position + quality
        # value, continuous across every state -- the displayed marker and
        # the logged trajectory below both read THIS, not bbox/predicted_pos
        # directly (module docstring: "the single product"). Subsumes Stage
        # 2's guarded fusion as the confirmed-TRACKING branch (computed once
        # here, not duplicated, so fusion_disagreement_streak only ever
        # advances once per frame).
        final_pos, final_quality, dominant_signal = None, None, "none"
        ...
        if state == "TRACKING" and not probation and bbox_has_valid_shape(bbox):
            ...
            if consensus_pos is not None:
                ...
                # before handing it to the guard -- see
                # CONSENSUS_RELIABILITY_SMOOTHING_WINDOW: raw, single-frame
                # reliability was measured to swing too much for the
                # persistence counter to ever accumulate, masking a real,
                # sustained disagreement behind short-term noise.
                consensus_reliability_history.append(consensus_rel)
                del consensus_reliability_history[:-CONSENSUS_RELIABILITY_SMOOTHING_WINDOW]
                smoothed_consensus_rel = sum(consensus_reliability_history) / len(consensus_reliability_history)

                fused_pos, corrected, fusion_disagreement_streak = compute_fused_position(
                    vit_pos, vit_rel, consensus_pos, smoothed_consensus_rel, fusion_disagreement_streak)
                if corrected:
                    n_fusion_corrections += 1
                    pull = math.hypot(fused_pos[0] - vit_pos[0], fused_pos[1] - vit_pos[1])
                    print(f"Frame {frame_idx} | FUSION CORRECTED | vit=({vit_pos[0]:.1f},{vit_pos[1]:.1f}) "
                          f"vit_rel={vit_rel:.2f} | consensus=({consensus_pos[0]:.1f},{consensus_pos[1]:.1f}) "
                          f"consensus_rel={consensus_rel:.2f} (smoothed={smoothed_consensus_rel:.2f}) | "
                          f"fused=({fused_pos[0]:.1f},{fused_pos[1]:.1f}) pulled {pull:.1f}px toward consensus | "
                          f"streak={fusion_disagreement_streak}")
                else:
                    print(f"Frame {frame_idx} | Fusion check | vit=({vit_pos[0]:.1f},{vit_pos[1]:.1f}) "
                          f"vit_rel={vit_rel:.2f} | consensus=({consensus_pos[0]:.1f},{consensus_pos[1]:.1f}) "
                          f"consensus_rel={consensus_rel:.2f} (smoothed={smoothed_consensus_rel:.2f}) | "
                          f"fused=vit (uncorrected) | streak={fusion_disagreement_streak}")
                final_pos = fused_pos
                final_quality = smoothed_consensus_rel if corrected else vit_rel
                dominant_signal = "vit+consensus (fused)" if corrected else "vit"
            else:
                fusion_disagreement_streak = 0
                consensus_reliability_history.clear()
                final_pos = vit_pos
                final_quality = vit_rel
                dominant_signal = "vit"

        elif state == "TRACKING" and not probation and bbox is not None:
            # VitTrack reported a degenerate (0,0,0,0) box -- its well-known
            # failure mode. That box's "center" is (0,0), a meaningless
            # frame-corner point, not a real position -- fusing against it
            # would treat total tracker failure as a legitimate signal to
            # correct away from. Fall back to consensus alone if available.
            fusion_disagreement_streak = 0
            consensus_reliability_history.clear()
            probation_fusion_streak = 0
            probation_consensus_reliability_history.clear()
            ...
            if consensus_pos is not None:
                ...
                dominant_signal = "consensus (vit degenerate)"

        elif state == "TRACKING" and probation and bbox_has_valid_shape(bbox):
            # Guarded fusion extended here too (per-user request): VitTrack's
            # own regression on a just-reinitialized candidate can be
            # noisier than a supporter consensus that's been tracked
            # continuously and independently of whether this candidate is
            # even the right object -- still gated by a disagreement/margin
            # guard (never a low-reliability signal moving the output), but
            # with its OWN streak/history (a confirmed-tracking guard's
            # timing shouldn't be affected by a brief, separate TENTATIVE
            # episode), a shorter persistence bar, and LOOSER disagreement/
            # margin thresholds than confirmed tracking (see
            # PROBATION_FUSION_PERSISTENCE_FRAMES and PROBATION_FUSION_MIN_*)
            # -- broadened here specifically because TENTATIVE candidates'
            # own reliability sits in a lower range to begin with.
            fusion_disagreement_streak = 0
            consensus_reliability_history.clear()
            vit_pos = (bbox[0] + bbox[2] / 2, bbox[1] + bbox[3] / 2)
            ...
            if consensus_pos is not None:
                ...
                smoothed_p = sum(probation_consensus_reliability_history) / len(probation_consensus_reliability_history)

                fused_pos, corrected, probation_fusion_streak = compute_fused_position(
                    vit_pos, vit_rel_p, consensus_pos, smoothed_p, probation_fusion_streak,
                    min_disagreement=PROBATION_FUSION_MIN_DISAGREEMENT_PX,
                    min_reliability_margin=PROBATION_FUSION_MIN_RELIABILITY_MARGIN,
                    persistence_frames=PROBATION_FUSION_PERSISTENCE_FRAMES)
                if corrected:
                    n_probation_fusion_corrections += 1
                    pull = math.hypot(fused_pos[0] - vit_pos[0], fused_pos[1] - vit_pos[1])
                    print(f"Frame {frame_idx} | FUSION CORRECTED (tentative) | vit=({vit_pos[0]:.1f},"
                          f"{vit_pos[1]:.1f}) vit_rel={vit_rel_p:.2f} | "
                          f"consensus=({consensus_pos[0]:.1f},{consensus_pos[1]:.1f}) "
                          f"consensus_rel={consensus_rel:.2f} (smoothed={smoothed_p:.2f}) | "
                          f"fused=({fused_pos[0]:.1f},{fused_pos[1]:.1f}) pulled {pull:.1f}px toward consensus | "
                          f"streak={probation_fusion_streak}")
                else:
                    print(f"Frame {frame_idx} | Fusion check (tentative) | vit=({vit_pos[0]:.1f},"
                          f"{vit_pos[1]:.1f}) vit_rel={vit_rel_p:.2f} | "
                          f"consensus=({consensus_pos[0]:.1f},{consensus_pos[1]:.1f}) "
                          f"consensus_rel={consensus_rel:.2f} (smoothed={smoothed_p:.2f}) | "
                          f"fused=vit (uncorrected) | streak={probation_fusion_streak}")
                final_pos = fused_pos
        ...
        elif state == "TRACKING" and probation and bbox is not None:
            # Same degenerate-box fallback as above, for the TENTATIVE case.
            fusion_disagreement_streak = 0
            consensus_reliability_history.clear()
            probation_fusion_streak = 0
            probation_consensus_reliability_history.clear()
            ...
            if consensus_pos is not None:
                final_pos = consensus_pos
                final_quality = consensus_rel
                dominant_signal = "consensus (vit degenerate, tentative)"
            ...
            if predicted_pos is not None:
                ...
                dominant_signal = "gmc (motion prediction)"
        ...
        if final_pos is not None:
            q_str = f"{final_quality:.2f}" if final_quality is not None else "N/A"
            print(f"Frame {frame_idx} | FINAL OUTPUT | pos=({final_pos[0]:.1f},{final_pos[1]:.1f}) "
                  f"quality={q_str} dominant={dominant_signal}")
